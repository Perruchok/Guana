...
export default function EntrarPage() {
  ...
  return (
    <div className="min-h-screen bg-brand-bg px-4 py-12 flex items-center justify-center">
      <div className="max-w-sm w-full">
        <Link href="/" className="font-extrabold tracking-tight text-2xl text-gray-900 block mb-10 text-center">
          Guana<span className="text-brand-blue-light">·</span>
        </Link>

        <h1 className="font-bold tracking-tight text-2xl text-center mb-1">Entrar</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mb-6">
          <div>
            ...
            <input
              ...
              disabled={loading}
            />
            <p className="text-xs text-slate-500 mt-1">💡 Usa tu nombre de usuario, no tu correo electrónico</p>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand-blue text-white font-medium py-3 rounded-sm hover:bg-brand-blue-light transition-colors disabled:opacity-50"
          >
        ...
        <p className="text-center text-sm text-slate-500">
          ¿No tienes cuenta?{' '}
          <Link href="/registro" className="text-brand-blue-light hover:underline font-medium">
            Regístrate
          </Link>
        </p>
      </div>
